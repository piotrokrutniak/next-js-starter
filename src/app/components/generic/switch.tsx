...
export default function Switch({value = false, setValue, className = "", label, switchColor}: {
    ...
    return(
        <div className={`${className} flex flex-col relative w-fit gap-1 justify-center items-end shrink-0 text-white`}>
            ...
            <div className="h-fit mr-0 w-12 p-1 shrink-0 rounded-xl cursor-pointer bg-slate-500/50 flex items-center" onClick={() => setValue(!value)}>
                <button className={` ${value ? "w-10" : "w-5"} w-10 items-center relative flex h-5 transition-all group`}>
